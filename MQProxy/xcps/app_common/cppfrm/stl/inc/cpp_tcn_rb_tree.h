...
修改记录:
************************************************************************************/
#ifndef __CPP_TCN_RB_TREE_H_
#define __CPP_TCN_RB_TREE_H_
...
//模块
template <class Key, class Value, class KeyOfValue, class Compare, class Alloc >  class rb_tree
{
protected:
    ...
    typedef __rb_tree_color_type    color_type;
    //节点大小
    enum {eNodeSize = sizeof(rb_tree_node)};
...
public:
    ...
    rb_tree(const Compare& comp = Compare())
        :node_count(0), key_compare(comp),node_alloctor(eNodeSize)
    {
        init();
    }
    //当使用内存预分配的配置器构造函数的接口.该接口ZZT添加
    rb_tree(size_t count,const Compare& comp = Compare())
        :node_count(0), key_compare(comp) ,node_alloctor(count+1,eNodeSize) //init函数中会多占一个节点
    {
        init();
    }
    ...
    rb_tree(const rb_tree<Key, Value, KeyOfValue, Compare, Alloc>& x)
        :node_count(0), key_compare(x.key_compare),node_alloctor(x.node_alloctor)
    {
        header = get_node();
...
public:
    ...
    XVOID swap(rb_tree<Key, Value, KeyOfValue, Compare, Alloc>& t)
    {
        std::swap(header, t.header);
        std::swap(node_count, t.node_count);
        std::swap(key_compare, t.key_compare);
        node_alloctor.swap(node_alloctor);
    }

public:
    //插入和删除操作.允许插入VALUE重复的元素
    std::pair<iterator,bool> insert_unique(const value_type& x);
    iterator insert_equal(const value_type& x);
    iterator insert_unique(iterator position, const value_type& x);
    iterator insert_equal(iterator position, const value_type& x);
    template <class InputIterator>
    XVOID insert_unique(InputIterator first, InputIterator last)
    {
    ...
    template <class InputIterator>
    XVOID insert_equal(InputIterator first, InputIterator last)
    {
        for ( ; first != last; ++first)
        {
            insert_equal(*first);
    ...
    XVOID erase(const key_type* first, const key_type* last);
    XVOID clear()
    {
        if (node_count != 0)
        {
...
template <class Key, class Value, class KeyOfValue, class Compare, class Alloc>
inline XVOID swap(rb_tree<Key, Value, KeyOfValue, Compare, Alloc>& x,
...
//typename rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::iterator 在VC上编译不过
template <class Key, class Value, class KeyOfValue, class Compare, class Alloc>
...
//清除操作，不用保持平衡。删除顺序<右,根,左>
template <class Key, class Value, class KeyOfValue, class Compare, class Alloc>
XVOID rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::__erase(link_type x)
{
    // erase without rebalancing
    while (x != 0)
    {
        __erase(right(x));//先删右子树
        link_type y = left(x);//临时保存左子树的指针
        destroy_node(x);//删除根节点
        x = y;
    }
}

template <class Key, class Value, class KeyOfValue, class Compare, class Alloc>
XVOID rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::erase(iterator first,
        iterator last)
{
...
template <class Key, class Value, class KeyOfValue, class Compare, class Alloc>
XVOID rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::erase(const Key* first,
        const Key* last)
{
    while (first != last) erase(*first++);
}
...
template <class Key, class Value, class KeyOfValue, class Compare, class Alloc>
inline typename std::pair<typename rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::iterator,
       typename rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::iterator>
...
inline typename std::pair<typename rb_tree<Key, Value, KoV, Compare, Alloc>::const_iterator,
...
inline XS32 __black_count(__rb_tree_node_base* node, __rb_tree_node_base* root)
{
    if (node == 0)
        return 0;
    else
    {
        XS32 bc = node->color == __rb_tree_black ? 1 : 0;
        if (node == root)
            return bc;
        else
...
rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::__rb_verify() const
{
    ...
    XS32 len = __black_count(leftmost(), root());
