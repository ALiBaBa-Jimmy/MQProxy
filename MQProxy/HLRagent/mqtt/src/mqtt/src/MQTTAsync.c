...
void MQTTAsync_lock_mutex(mutex_type amutex)
{
	...
	if (rc != 0)
		Log(LOG_ERROR_MQTT, 0, "Error %s locking mutex", strerror(rc));
}
...
void MQTTAsync_unlock_mutex(mutex_type amutex)
{
	...
	if (rc != 0)
		Log(LOG_ERROR_MQTT, 0, "Error %s unlocking mutex", strerror(rc));
}
...
int MQTTAsync_unpersistCommand(MQTTAsync_queuedCommand* qcmd)
{
	...
	if ((rc = qcmd->client->c->persistence->premove(qcmd->client->c->phandle, key)) != 0)
		Log(LOG_ERROR_MQTT, 0, "Error %d removing command from persistence", rc);
	FUNC_EXIT_RC(rc);
	return rc;
}
...
int MQTTAsync_persistCommand(MQTTAsync_queuedCommand* qcmd)
{
	...
	if (nbufs > 0)
	{
		if ((rc = aclient->c->persistence->pput(aclient->c->phandle, key, nbufs, (char**)bufs, lens)) != 0)
			Log(LOG_ERROR_MQTT, 0, "Error persisting command, rc %d", rc);
...
thread_return_type WINAPI MQTTAsync_sendThread(void* n)
{
	...
	while (!tostop)
	{
		...
		if ((rc = Thread_wait_cond(send_cond, 1)) != 0 && rc != ETIMEDOUT)
			Log(LOG_ERROR_MQTT, -1, "Error %d waiting for condition variable", rc);
#else
		if ((rc = Thread_wait_sem(send_sem, 1000)) != 0 && rc != ETIMEDOUT)
			Log(LOG_ERROR_MQTT, -1, "Error %d waiting for semaphore", rc);
#endif
...
void MQTTAsync_destroy(MQTTAsync* handle)
{
	...
	if (!ListRemove(handles, m))
		Log(LOG_ERROR_MQTT, -1, "free error");
	*handle = NULL;
...
thread_return_type WINAPI MQTTAsync_receiveThread(void* n)
{
	...
	while (!tostop)
	{
		...
		if (m == NULL)
		{
			Log(LOG_ERROR_MQTT, -1, "Client structure was NULL for socket %d - removing socket", sock);
			Socket_close(sock);
			continue;
		}
		if (rc == SOCKET_ERROR)
			...
			if (pack)
			{
					...
					while (ListNextElement(m->responses, &current))
					{
						...
						{	
							...
							if (!ListDetach(m->responses, command)) /* remove the response from the list */
								Log(LOG_ERROR_MQTT, -1, "Subscribe command not removed from command list");
				...
				else if (pack->header.bits.type == UNSUBACK)
				{
					...
					while (ListNextElement(m->responses, &current))
					{
						...
						{		
							if (!ListDetach(m->responses, command)) /* remove the response from the list */
								Log(LOG_ERROR_MQTT, -1, "Unsubscribe command not removed from command list");
							if (command->command.onSuccess)
							{
...
int MQTTAsync_cleanSession(Clients* client)
{
	...
	if ((found = ListFindItem(handles, client, clientStructCompare)) != NULL)
	{
		...
		Log(LOG_ERROR_MQTT, -1, "cleanSession: did not find client structure in handles list");
	FUNC_EXIT_RC(rc);
	return rc;
}
...
void Protocol_processPublication(Publish* publish, Clients* client)
{
	...
	if (client->messageQueue->count == 0 && client->connected)
	{
		...
		if ((found = ListFindItem(handles, client, clientStructCompare)) == NULL)
			Log(LOG_ERROR_MQTT, -1, "processPublication: did not find client structure in handles list");
		else
		{
			MQTTAsyncs* m = (MQTTAsyncs*)(found->content);
		...
		if (pack)
		{
			...
			else if (pack->header.bits.type == PUBACK || pack->header.bits.type == PUBCOMP)
			{
				...
				*rc = (pack->header.bits.type == PUBCOMP) ?
				...
				if (!m)
					Log(LOG_ERROR_MQTT, -1, "PUBCOMP or PUBACK received for no client, msgid %d", msgid);
				if (m)
				{
					ListElement* current = NULL;
					...
					while (ListNextElement(m->responses, &current))
					{
						...
						{		
							if (!ListDetach(m->responses, command)) /* then remove the response from the list */
								Log(LOG_ERROR_MQTT, -1, "Publish command not removed from command list");
							if (command->command.onSuccess)
							{
								MQTTAsync_successData data;
