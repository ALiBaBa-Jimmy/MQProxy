...
void checkEyecatchers(char* file, int line, void* p, int size)
{
	...
	if ((us = *--sp) != eyecatcher)
		Log(LOG_ERROR_MQTT, 13, msg, "start", us, file, line);

	cp += size;
	if ((us = *(int*)cp) != eyecatcher)
		Log(LOG_ERROR_MQTT, 13, msg, "end", us, file, line);
}
...
int Internal_heap_unlink(char* file, int line, void* p)
{
	...
	if (e == NULL)
		Log(LOG_ERROR_MQTT, 13, "Failed to remove heap item at file %s line %d", file, line);
	else
	{
...
void *myrealloc(char* file, int line, void* p, size_t size)
{
	...
	if (s == NULL)
		Log(LOG_ERROR_MQTT, 13, "Failed to reallocate heap item at file %s line %d", file, line);
	else
	{
		int space = sizeof(storageElement);
		int filenamelen = strlen(file)+1;
...
void Heap_terminate()
{
	...
	{
		Log(LOG_ERROR_MQTT, -1, "Some memory not freed at shutdown, possible memory leak");
		HeapScan(LOG_ERROR_MQTT);
	}
}
