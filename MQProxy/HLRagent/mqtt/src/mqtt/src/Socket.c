...
int Socket_addSocket(int newSd)
{
	...
	if (ListFindItem(s.clientsds, &newSd, intcompare) == NULL) /* make sure we don't add the same socket twice */
	{
		...
		*pnewSd = newSd;
		...
		Log(LOG_ERROR_MQTT, -1, "addSocket: socket %d already in the list", newSd);
...
void Socket_close(int socket)
{
	...
	if (ListRemoveItem(s.clientsds, &socket, intcompare))
		...
		Log(LOG_ERROR_MQTT, -1, "Failed to remove socket %d", socket);
	...
	*sock = -1;
	...
	if ((rc = getaddrinfo(addr, NULL, &hints, &result)) == 0)
	{
		...
		freeaddrinfo(result);
	  	...
	  	Log(LOG_ERROR_MQTT, -1, "getaddrinfo failed for addr %s with rc %d", addr, rc);

	if (rc != 0)
		Log(LOG_ERROR_MQTT, -1, "%s is not a valid IP address", addr);
	else
	{
		*sock =	socket(family, type, 0);
		...
		else
		{
			...
			if (setsockopt(*sock, SOL_SOCKET, SO_NOSIGPIPE, (void*)&opt, sizeof(opt)) != 0)
				Log(LOG_ERROR_MQTT, -1, "Could not set SO_NOSIGPIPE for socket %d", *sock);
#endif
