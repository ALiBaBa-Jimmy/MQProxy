...
ifeq ($(MAKE_PLATFORM), WIN32)
  ...
  ifeq ($(MAKE_PLATFORM), LINUX)
    PRODUCT_NAME       = $(TEST_EXE)
  endif
endif
MAKEFILE_NAME       = $(SRC_ROOT)/test/test.mak
...
ifeq ($(MAKE_TYPE), SRC_TYPE)
  ...
  LIB_PATH            = 	
	OBJS		     = $(PRG_ROOT)/bin/linux/test.o
	
	MAKES_LIST       += $(SRC_ROOT)/test/makefile
...
ifeq ($(MAKE_PLATFORM), WIN32)
  ...
  LIB_OBJS  = -L$(dir ${ORACLE_LIB})/ -ldl -lnsl -lclntsh  -L$(PRG_ROOT)/bin/linux -locci -ldbproj -lstdc++
endif
